...
num_ratings = st.slider('***Number of Ratings***', 0, 1500000,1000)
...
back_cam_score = st.number_input('***Back Camera Score***', min_value=0, step=1)
front_cam_score = st.number_input('***Front Camera Score***', min_value=0, step=1)
Main_Back_Camera= st.selectbox('***Main_Back_Camera***', np.sort(df['Main_Back_Camera'].unique()))
Main_Front_Camera=st.selectbox('***Main_Front_Camera***', np.sort(df['Main_Front_Camera'].unique()))
    # Predict the price
